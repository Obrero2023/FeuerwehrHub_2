...
export async function renderVehicleBookings() {
  ...
  const renderBookings = () => {
    ...
    grid.innerHTML = `<table class="data-table">
      <thead><tr>
        <th>Fahrzeug</th><th>Datum</th><th>Von</th><th>Bis</th><th>Grund</th><th style="color:var(--text-color)">Status</th><th>Buchungs-ID</th><th>Gebucht von</th><th>Bestätigt von</th><th>Abgesagt von</th>
      ...
      <tbody>
        ${bookings.map(b => {
          ...
          <tr data-id="${b.id}" style="--status-color: ${statusColor}">
            ...
            <td style="font-size:0.85em;color:var(--text-muted)">${b.status === 'abgesagt' && b.status_changed_by_name ? esc(b.status_changed_by_name) : '—'}</td>
            ${canManage ? `<td>
