...
pub async fn create_vehicle(
    ...
    let vehicle_type = validate_vehicle_type(&state.db, body.vehicle_type.as_deref()).await?;
...
// ── Fahrzeugtypen (dynamisch aus DB) ─────────────────────────────────────────
...
pub async fn list_vehicle_types(
    State(state): State<AppState>,
) -> AppResult<Json<Vec<VehicleType>>> {
    let rows = sqlx::query_as::<_, VehicleType>(
        "SELECT id, key, label, sort_order FROM vehicle_types ORDER BY sort_order ASC, label ASC"
    )
    .fetch_all(&state.db)
    .await?;
    Ok(Json(rows))
...
pub async fn create_vehicle_type(
    State(state): State<AppState>,
    Json(body): Json<VehicleTypeBody>,
) -> AppResult<Json<VehicleType>> {
    ...
    let row = sqlx::query_as::<_, VehicleType>(
        "INSERT INTO vehicle_types (key, label, sort_order) VALUES ($1, $2, $3)
    ...
    .map_err(|e| {
        if e.to_string().contains("duplicate key") || e.to_string().contains("unique") {
            AppError::BadRequest(format!("Fahrzeugtyp '{}' existiert bereits", key))
        } else {
            AppError::from(e)
        }
    })?;
...
pub async fn update_vehicle_type(
    ...
    if key.is_empty() || label.is_empty() {
        return Err(AppError::BadRequest("Key und Label duerfen nicht leer sein".into()));
    }

    let row = sqlx::query_as::<_, VehicleType>(
        "UPDATE vehicle_types SET key=$1, label=$2, sort_order=$3 WHERE id=$4
         RETURNING id, key, label, sort_order"
    )
    .bind(&key)
    .bind(&label)
...
pub async fn delete_vehicle_type(
    ...
    // Pruefen ob noch Fahrzeuge diesen Typ verwenden
    let count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM vehicles v
         JOIN vehicle_types vt ON vt.key = v.vehicle_type
         WHERE vt.id = $1"
    )
    .bind(id)
    .fetch_one(&state.db)
    ...
    if count > 0 {
        return Err(AppError::BadRequest(
            format!("Fahrzeugtyp wird noch von {} Fahrzeug(en) verwendet", count)
        ));
    }

    sqlx::query("DELETE FROM vehicle_types WHERE id = $1")
        .bind(id)
        .execute(&state.db)
        .await?;
...
async fn validate_vehicle_type(pool: &sqlx::PgPool, t: Option<&str>) -> AppResult<String> {
    let key = t.unwrap_or("lkw");
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM vehicle_types WHERE key = $1)"
    )
    .bind(key)
    .fetch_one(pool)
    .await?;
    ...
    if !exists {
        return Err(AppError::BadRequest(format!("Ungültiger Fahrzeugtyp: {}", key)));
    }
    Ok(key.to_string())
}
...
pub fn router(state: AppState) -> Router<AppState> {
    Router::new()
        .route("/types",                                    get(list_vehicle_types).post(create_vehicle_type))
        .route("/types/:tid",                               put(update_vehicle_type).delete(delete_vehicle_type))
